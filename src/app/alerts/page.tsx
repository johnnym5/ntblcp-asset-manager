...
/**
 * @fileOverview Tactical Alerts Cockpit - High-Risk Exception Workspace.
 * Phase 59: Dedicated management of Stolen, Burnt, and Unsalvageable asset pulses.
 */
...
export default function AlertsCockpitPage() {
  ...
  const stats = useMemo(() => {
    const stolen = criticalAssets.filter(a => a.condition === 'Stolen').length;
    const damaged = criticalAssets.filter(a => ['Burnt', 'Unsalvageable'].includes(a.condition || '')).length;
    const discrepancies = criticalAssets.filter(a => a.status === 'DISCREPANCY').length;
    return { stolen, damaged, discrepancies, total: criticalAssets.length };
  }, [criticalAssets]);
    ...
    <AppLayout>
      <div className="space-y-10 animate-in fade-in duration-700 pb-32 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
          <div className="space-y-2">
            ...
            <p className="font-bold uppercase text-[10px] tracking-[0.3em] text-muted-foreground opacity-70">
              High-Risk Exception Management & Critical Asset Pulses
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-2">
          <Card className="rounded-[2.5rem] border-2 border-destructive/20 shadow-xl bg-destructive/[0.02]">
            <CardHeader className="pb-2">
              <CardTitle className="text-[10px] font-black uppercase tracking-widest text-destructive flex items-center gap-2">
                <XCircle className="h-3.5 w-3.5" /> Stolen Pulses
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-5xl font-black tracking-tighter text-destructive">{stats.stolen}</div>
              <p className="text-[9px] font-bold text-muted-foreground uppercase mt-2 opacity-60">Reported physical losses</p>
            </CardContent>
          </Card>

          <Card className="rounded-[2.5rem] border-2 border-orange-500/20 shadow-xl bg-orange-500/[0.02]">
            <CardHeader className="pb-2">
              <CardTitle className="text-[10px] font-black uppercase tracking-widest text-orange-600 flex items-center gap-2">
                <Bomb className="h-3.5 w-3.5" /> Damaged / Burnt
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-5xl font-black tracking-tighter text-orange-600">{stats.damaged}</div>
              <p className="text-[9px] font-bold text-muted-foreground uppercase mt-2 opacity-60">Critical integrity failures</p>
            </CardContent>
          </Card>

          <Card className="rounded-[2.5rem] border-2 border-primary/20 shadow-xl bg-primary/[0.02]">
            <CardHeader className="pb-2">
            ...
            <CardContent>
              ...
              <p className="text-[9px] font-bold text-muted-foreground uppercase mt-2 opacity-60">Awaiting management resolution</p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 px-2">
          <div className="relative flex-1 w-full group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground opacity-40 group-focus-within:text-destructive transition-colors" />
            <Input 
              placeholder="Search critical exceptions..." 
              className="pl-14 h-16 rounded-[1.5rem] bg-card border-none shadow-xl font-bold text-sm focus-visible:ring-destructive/20 transition-all placeholder:opacity-30"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <Button variant="outline" className="h-16 px-10 rounded-[1.5rem] font-black uppercase text-[10px] tracking-widest gap-3 bg-card border-none shadow-xl transition-all hover:bg-destructive/5">
          ...
          {filteredAlerts.length > 0 ? (
            ...
            <div className="py-40 text-center opacity-20 flex flex-col items-center gap-8 border-4 border-dashed rounded-[4rem]">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-medium italic max-w-sm mx-auto">No high-risk asset exceptions discovered in current pulse.</p>
              </div>
            </div>
          )}
        </div>
