      ...
      <div className="space-y-6 flex flex-col h-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Button variant="outline" className="h-12 px-6 rounded-2xl font-black uppercase text-[10px] tracking-widest gap-2 bg-card border-none shadow-sm" onClick={() => setIsFilterOpen(!isFilterOpen)}>
        ...
        <div className="flex-1 bg-card/50 rounded-3xl border border-border/40 overflow-hidden shadow-2xl flex flex-col">
                    ...
                    <tr key={asset.id} className="hover:bg-primary/[0.02] transition-colors group cursor-pointer">
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          <span className="text-sm font-black tracking-tight">{asset.description}</span>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] text-muted-foreground font-medium truncate max-w-[120px]">{asset.assetIdCode || 'No Tag'}</span>
                        </div>
                      </td>
                      <td className="py-4 px-6">
                      ...
                      <td className="py-4 px-6">
                        <div className={cn(
                          ...
                          asset.verifiedStatus === 'Verified' ? "text-green-500" : "text-orange-500"
                        )}>
                          <div className={cn("h-1.5 w-1.5 rounded-full animate-pulse", asset.verifiedStatus === 'Verified' ? "bg-green-500" : "bg-orange-500")} />
          ...
          <div className="mt-auto p-6 bg-muted/10 border-t border-border/40 flex items-center justify-between">
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">
              Showing {assets.length} active records
            </span>
