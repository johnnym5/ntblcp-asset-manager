...
export function ErrorAuditWorkstation() {
  ...
  const handleExportErrors = () => {
    ...
    saveAs(blob, `System-Health-Export-${new Date().toISOString().split('T')[0]}.json`);
  };
    ...
    <div className="max-w-6xl mx-auto space-y-10 pb-32 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-2">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <FileJson className="h-4 w-4" /> Export Health Log
          </Button>
          <Button variant="outline" onClick={loadLogs} className="h-14 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest gap-3 border-2 border-white/5 hover:bg-white/5 transition-all text-white">
            <Activity className="h-4 w-4" /> Refresh Log
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 px-2">
        ...
        <Card className={cn("rounded-[2rem] border-2 shadow-2xl bg-black/40", stats.critical > 0 ? "border-destructive/40" : "border-white/5")}>
          ...
          <CardContent>
            ...
            <p className="text-[9px] font-bold text-muted-foreground uppercase mt-2 opacity-60">Severe connectivity interruptions</p>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-[2rem] border-2 border-white/5 shadow-2xl bg-black/40">
          ...
          <CardContent>
            ...
            <p className="text-[9px] font-bold text-muted-foreground uppercase mt-2 opacity-60">Awaiting admin resolution</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4 px-2">
        {loading ? (
          <div className="py-40 flex flex-col items-center justify-center gap-6 opacity-40">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.3em] text-white">Retrieving Health Data...</p>
          </div>
          ...
          <div className="py-40 text-center opacity-30 flex flex-col items-center gap-8">
            <div className="p-16 bg-white/5 rounded-[4rem] shadow-inner">
              <ShieldCheck className="h-28 w-24 text-muted-foreground" />
            </div>
          ...
          <div className="p-8 bg-white/5 border-b border-white/5 space-y-2">
            <DialogHeader>
              <div className="flex items-center justify-between">
                <DialogTitle className="text-2xl font-black uppercase tracking-tight text-white">Incident Breakdown</DialogTitle>
              ...
              <DialogDescription className="font-bold uppercase text-[10px] tracking-widest opacity-60 text-white/40">Technical Failure Analysis</DialogDescription>
            </DialogHeader>
          </div>
          ...
          <ScrollArea className="max-h-[60vh] bg-black">
            <div className="p-8 space-y-10">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Module</h4>
                  <p className="text-sm font-bold text-white">{selectedLog?.context.module}</p>
                </div>
                <div className="space-y-2 text-right">
                  <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Action</h4>
                  <p className="text-sm font-bold text-white">{selectedLog?.context.action}</p>
                </div>
              </div>

              <div className="space-y-4">
                <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40">Stack Trace</h4>
              ...
              <div className="p-6 rounded-3xl bg-blue-500/5 border-2 border-dashed border-blue-500/20 flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs font-bold text-white">{selectedLog?.recovery.attempted ? `Executed: ${selectedLog.recovery.action}` : 'Manual Resolution Required'}</p>
                </div>
