...
export function GISWorkstation() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'DISCREPANCY': return 'bg-destructive';
  ...
  return (
    <div className="h-[calc(100vh-10rem)] flex flex-col gap-6 animate-in fade-in duration-700">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-hidden">
        ...
        <Card className="lg:col-span-8 rounded-[3rem] border-2 border-border/40 shadow-2xl bg-[#0F172A] overflow-hidden relative group">
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
            <div className="w-[80%] h-[80%] border border-primary/5 rounded-full animate-[pulse_4s_ease-in-out_infinite]" />
            <div className="absolute w-[60%] h-[60%] border border-primary/5 rounded-full animate-[pulse_6s_ease-in-out_infinite]" />
          </div>
          ...
          <div className="relative h-full flex flex-col p-10">
            <div className="flex items-center justify-between mb-8">
              ...
              <Badge className="bg-primary/20 text-primary border-primary/20 font-black h-6 px-3 uppercase text-[8px] tracking-widest">Protocol: Active Pulse</Badge>
            </div>

            <div className="flex-1 relative flex items-center justify-center overflow-hidden border-2 border-dashed border-white/5 rounded-[2rem]">
              <AnimatePresence>
                {filteredAssets.slice(0, 150).map((asset, idx) => (
                  <motion.button
                    key={asset.id}
                  ...
                  >
                    <div className="relative">
                      <div className={cn(
                        "h-3 w-3 rounded-full border-2 border-black transition-all group-hover/pin:scale-150 group-hover/pin:z-50 shadow-lg",
                        getStatusColor(asset.status)
                      )} />
                      <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 opacity-0 group-hover/pin:opacity-100 transition-all bg-white text-black p-3 rounded-xl shadow-2xl whitespace-nowrap z-50 pointer-events-none translate-y-2 group-hover/pin:translate-y-0">
              ...
              {filteredAssets.length === 0 && (
                <div className="flex flex-col items-center gap-4 opacity-20">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white">No Spatial Pulses Detected</span>
                </div>
              )}
            </div>
            ...
            <div className="mt-8 flex items-center justify-between text-white/40">
              <div className="flex flex-wrap items-center gap-6 text-[8px] font-black uppercase tracking-[0.3em]">
                ...
                <span className="flex items-center gap-2"><div className="h-1.5 w-1.5 rounded-full bg-white/40" /> Pending Pulse</span>
                <span className="flex items-center gap-2"><div className="h-1.5 w-1.5 rounded-full bg-destructive" /> Discrepancy</span>
              </div>
              <p className="hidden sm:block text-[8px] font-mono uppercase tracking-widest opacity-20">GRID_RES: 32PX_DETERMINISTIC</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="lg:col-span-4 rounded-[3rem] border-2 border-border/40 shadow-2xl bg-card/50 overflow-hidden flex flex-col">
          <CardHeader className="p-8 bg-muted/20 border-b space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-black uppercase tracking-tight">Spatial Register</h3>
              <Badge variant="outline" className="font-black text-[9px] h-6 px-3 border-primary/20 text-primary">
                {filteredAssets.length} ANCHORS
              </Badge>
            </div>
            <div className="relative group">
              ...
              <Input 
                placeholder="Scan Geotag Identity..." 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pl-9 h-11 rounded-xl bg-background border-none shadow-inner text-xs font-bold focus-visible:ring-primary/20"
              />
            </div>
          </CardHeader>
                ...
                >
                  <div className="flex items-center gap-4 min-w-0">
                    <div className={cn(
                      ...
                      asset.status === 'VERIFIED' ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground group-hover:bg-primary group-hover:text-primary-foreground"
                    )}>
                      <MapPin className="h-4 w-4" />
                    </div>
                    <div className="flex flex-col min-w-0">
                      <span className="text-xs font-black uppercase truncate leading-tight">{asset.description}</span>
                      <div className="flex items-center gap-2 text-[8px] font-bold text-muted-foreground uppercase opacity-60 tracking-tighter mt-0.5">
                        <span className="truncate">{asset.location}</span>
                        <span>•</span>
                        <span className="font-mono text-[7px]">{asset.geotag!.lat.toFixed(4)}, {asset.geotag!.lng.toFixed(4)}</span>
                      </div>
                    </div>
                  </div>
