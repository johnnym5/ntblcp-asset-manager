 ...
 * Phase 455: Implemented robust root scrolling and enhanced group padding.
 */
...
export function SyncQueueWorkstation({ isEmbedded = false }: { isEmbedded?: boolean }) {
  ...
  return (
    <div className={cn("space-y-10", !isEmbedded && "max-w-5xl mx-auto pb-40 animate-in fade-in duration-700")}>
      ...
      <Card className="bg-[#080808] border-2 border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl">
        ...
        <CardContent className="p-8 flex flex-col sm:flex-row items-center justify-between gap-10">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <Button variant="outline" onClick={manualDownload} disabled={isSyncing || !isOnline} className="flex-1 sm:flex-none h-14 px-8 rounded-2xl font-black uppercase text-[10px] tracking-widest border-2 hover:bg-white/5 transition-all text-white/60 shadow-sm">
      ...
      <AnimatePresence>
        {queue.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="px-2">
            <div className="flex items-center gap-4 px-8 py-5 bg-white/5 rounded-[1.5rem] border border-white/10 group transition-all hover:border-primary/20 shadow-xl">
              <Checkbox 
                id="sync-select-all" 
                checked={allSelected} 
      ...
      <div className="space-y-12 px-2">
        {sortedCategories.length > 0 ? (
                ...
                <div className="flex items-center justify-between px-4">
                  <div className="flex items-center gap-5">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div className="p-2.5 bg-white/5 rounded-xl shadow-inner"><Layers className="h-4 w-4 text-white/40" /></div>
                    </div>
                    <div className="space-y-0.5">
                      <h4 className="text-sm font-black uppercase tracking-widest text-white leading-none">{category}</h4>
                      <p className="text-[9px] font-bold text-white/20 uppercase tracking-[0.2em]">{items.length} PENDING PULSES</p>
                    </div>
                  </div>
                  {categorySelectedCount > 0 && (
                    <Badge variant="outline" className="h-6 px-3 border-primary/20 text-primary bg-primary/5 font-black text-[9px] tracking-widest">
                      {categorySelectedCount} IN GROUP
                    </Badge>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  {items.map((entry) => (
                    <Card key={entry.id} className={cn(
                      "bg-[#050505] border-2 rounded-[2rem] overflow-hidden transition-all group",
                      ...
                      <div className="flex items-center p-6 gap-8">
                        <Checkbox 
                          ...
                          className="h-6 w-6 rounded-full border-2 border-white/10 data-[state=checked]:bg-primary shadow-sm"
                        />
                        
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-4">
                            <h5 className="text-base font-black uppercase tracking-tight text-white truncate leading-none">
                              {(entry.payload as any).description || 'System Update Pulse'}
                            </h5>
                            <Badge variant="outline" className={cn(
                              "h-5 px-2.5 text-[8px] font-black uppercase tracking-[0.2em] rounded-md",
                          ...
                          <div className="flex items-center gap-6 mt-3 text-[9px] font-bold text-white/20 uppercase tracking-[0.2em]">
                            ...
                            <span className="flex items-center gap-2"><Tag className="h-3.5 w-3.5" /> ROW: {(entry.payload as any).sn || 'MANUAL'}</span>
                          </div>
                        </div>

                        <div className="flex items-center gap-2 shrink-0 opacity-20 group-hover:opacity-100 transition-opacity">
                          {entry.status === 'FAILED' && (
                            <Button variant="ghost" size="icon" onClick={() => handleRetry(entry)} className="h-11 w-11 rounded-xl bg-white/5 hover:bg-primary/10 text-primary transition-all shadow-md">
                              <RotateCcw className="h-5 w-5" />
                            </Button>
                          )}
                          <Button variant="ghost" size="icon" onClick={() => storage.dequeue(entry.id).then(loadQueue)} className="h-11 w-11 rounded-xl bg-white/5 text-destructive/40 hover:text-destructive hover:bg-destructive/10 transition-all shadow-md">
          ...
          <div className="py-48 text-center opacity-20 border-4 border-dashed border-white/5 rounded-[4rem] flex flex-col items-center gap-10 bg-white/[0.01] shadow-inner mx-2">
            <div className="relative">
              <div className="p-20 bg-white/5 rounded-[3rem] shadow-inner">
                <Database className="h-32 w-28 text-white" />
              </div>
              <div className="absolute -bottom-6 -right-6 p-8 bg-primary rounded-[2rem] shadow-2xl animate-pulse">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm font-medium italic text-white/60 max-w-sm mx-auto leading-relaxed">
